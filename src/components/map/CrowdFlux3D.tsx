...
export function CrowdFlux3D({ eventoCodigo = 'NEXT26', count = 56 }: CrowdFlux3DProps) {
  ...
  const adjacency = useMemo(() => {
    ...
    for (const e of edges) {
      ...
      adj.get(e.from)?.push({ to: e.to, distance: dist });
      adj.get(e.to)?.push({ to: e.from, distance: dist });
    }

    return adj;
  }, [waypoints, edges, waypointMap]);

  // Inicializa o estado dos pedestres distribuídos aleatoriamente pelos trechos da API
  ...
  useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      walkers.push({
        fromId,
        toId,
        progress: Math.random(), // espalhados ao longo do trecho
        speedMps: 1.2 + Math.random() * 0.8, // 1.2 a 2.0 m/s
        offsetLateral: (Math.random() - 0.5) * 0.7, // pequeno deslocamento lateral para não andar em fila única
  ...
  useFrame((_, delta) => {
    ...
    for (let i = 0; i < walkers.length; i++) {
      ...
      if (w.progress >= 1) {
        ...
        if (Math.random() < 0.15) {
          w.pauseTimer = 2.0 + Math.random() * 2.5;
        }

        if (neighbors.length > 0) {
          // Filtra o nó de onde acabou de vir (para seguir em frente se houver opções)
          const forwardOptions = neighbors.filter((n) => n.to !== w.fromId);
          const pickList = forwardOptions.length > 0 ? forwardOptions : neighbors;
          const next = pickList[Math.floor(Math.random() * pickList.length)];
