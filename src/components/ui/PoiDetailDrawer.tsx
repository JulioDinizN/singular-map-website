...
export function PoiDetailDrawer({
  ...
  return (
    <aside
      aria-label="Venue Point Details"
    ...
    >
      <div className="bg-slate-900/95 backdrop-blur-xl border border-slate-700/80 rounded-t-3xl sm:rounded-3xl shadow-2xl shadow-black/80 max-h-[75vh] sm:max-h-[80vh] flex flex-col overflow-hidden">
        ...
        <div className="p-4 sm:p-5 border-b border-slate-800/80 flex items-start justify-between gap-3">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1.5 flex-wrap">
              ...
              {poi.boothNumber && (
                <span className="text-xs text-sky-400 font-mono font-semibold bg-sky-950/50 px-2 py-0.5 rounded border border-sky-800/50">
                  Booth {poi.boothNumber}
                </span>
              )}
            </div>
          ...
          {poi.sessions && poi.sessions.length > 0 && (
            <div className="space-y-2 pt-1">
              <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
                ...
                <span>Today's Sessions & Talks</span>
              </h3>
        ...
        {/* Footer Actions */}
        <div className="p-4 sm:p-5 border-t border-slate-800/80 bg-slate-950/50 flex items-center gap-2.5">
          <button
            type="button"
            onClick={() => onNavigateHere(poi)}
            className="flex-1 flex items-center justify-center gap-2 bg-gradient-to-r from-sky-500 to-indigo-600 hover:from-sky-400 hover:to-indigo-500 text-white font-semibold py-3 px-4 rounded-2xl shadow-lg shadow-sky-500/30 text-xs sm:text-sm transition-all active:scale-[0.98]"
          >
          ...
          <button
            type="button"
            onClick={handleShare}
            className="p-3 rounded-2xl bg-slate-800/80 text-slate-400 hover:text-white border border-slate-700/80 transition-all"
